...
// Bag with two static TF topics, a dynamic /tf topic, and one cloud topic:
//   /tf_static          msg 1: base_link -> lidar (x=1)
//                       msg 2: base_link -> camera (x=2) + base_link -> lidar (x=3)
//   /sensing/tf_static  msg 1: map -> odom (x=4)
//   /tf                 msg 1: odom -> base_link (x=5, dynamic)
//   /points_a           1 dummy message (the scan needs a cloud topic)
std::filesystem::path build_multi_static_bag(const std::filesystem::path & path)
{
  auto writer = bagwiz::io::open_write(path, [] {
    bagwiz::io::CreateOptions opts;
    opts.format = bagwiz::io::Format::Mcap;
  ...
  writer->declare_topic(bagwiz::core::make_tf_message_topic_info("/tf_static"));
  writer->declare_topic(bagwiz::core::make_tf_message_topic_info("/sensing/tf_static"));
  writer->declare_topic(bagwiz::core::make_tf_message_topic_info("/tf"));
  writer->declare_topic(make_topic("/points_a", "sensor_msgs/msg/PointCloud2"));

  using Edges = std::vector<geometry_msgs::msg::TransformStamped>;
  writer->write(
    "/tf_static", 1'000'000'000LL,
    bagwiz::core::serialize_tf_message(Edges{make_edge("base_link", "lidar", 1.0)}));
  writer->write(
    "/sensing/tf_static", 1'050'000'000LL,
    bagwiz::core::serialize_tf_message(Edges{make_edge("map", "odom", 4.0)}));
  writer->write(
    "/tf", 1'080'000'000LL,
    bagwiz::core::serialize_tf_message(Edges{make_edge("odom", "base_link", 5.0)}));
  writer->write(
    "/tf_static", 1'100'000'000LL,
    bagwiz::core::serialize_tf_message(
      Edges{make_edge("base_link", "camera", 2.0), make_edge("base_link", "lidar", 3.0)}));
  writer->write("/points_a", 1'200'000'000LL, dummy_payload_view());
  writer->close();
  return path;
}

TEST_F(WalkOverlayScanTest, CollectsRawStaticTransformsPerTopic)
{
  const auto bag = build_multi_static_bag(tmp_dir_ / "multi_static.mcap");
  std::atomic<bool> cancel{false};
  OverlayScanResult result;
  ...
  EXPECT_TRUE(result.error.empty()) << result.error;
  ASSERT_EQ(result.static_transforms.size(), 2U);

  // Topics keep the order their first message appears in the bag.
  EXPECT_EQ(result.static_transforms[0].name, "/tf_static");
  ASSERT_EQ(result.static_transforms[0].transforms.size(), 2U);
  // Children keep first-seen order; a re-published child keeps its slot but
  // takes the newest value (last wins), mirroring core::collect_static_tf.
  EXPECT_EQ(result.static_transforms[0].transforms[0].child_frame_id, "lidar");
  EXPECT_DOUBLE_EQ(result.static_transforms[0].transforms[0].transform.translation.x, 3.0);
  EXPECT_EQ(result.static_transforms[0].transforms[1].child_frame_id, "camera");
  EXPECT_DOUBLE_EQ(result.static_transforms[0].transforms[1].transform.translation.x, 2.0);

  EXPECT_EQ(result.static_transforms[1].name, "/sensing/tf_static");
  ASSERT_EQ(result.static_transforms[1].transforms.size(), 1U);
  EXPECT_EQ(result.static_transforms[1].transforms[0].child_frame_id, "odom");
  EXPECT_DOUBLE_EQ(result.static_transforms[1].transforms[0].transform.translation.x, 4.0);

  // The dynamic /tf edge (child base_link) must not leak into the static set.
  for (const auto & st : result.static_transforms) {
    for (const auto & t : st.transforms) {
      EXPECT_NE(t.child_frame_id, "base_link");
    }
  }
}
