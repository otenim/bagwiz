...
class TopicKeepTest : public ::testing::Test
{
protected:
  void SetUp() override
  {
    tmp_dir_ =
      ...
      ("bagwiz_topic_keep_" + std::to_string(::testing::UnitTest::GetInstance()->random_seed()) +
       "_" +
       std::to_string(
...
TEST_F(TopicKeepTest, KeepExactTopicToOutput)
{
  ...
  ASSERT_EQ(out.count("/sensing/lidar"), 1U);  // the only survivor
  EXPECT_EQ(out.at("/sensing/lidar"), 1);
  EXPECT_EQ(out.count("/sensing/camera"), 0U);  // dropped: not even declared
...
TEST_F(TopicKeepTest, KeepMultipleTopicsInSubtree)
{
  ...
  EXPECT_EQ(out.at("/sensing/lidar"), 1);
  EXPECT_EQ(out.count("/perception/objects"), 0U);  // outside the kept subtree
  EXPECT_EQ(out.size(), 2U);
}

TEST_F(TopicKeepTest, KeepMultipleSelectors)
{
  const auto in_path = build_input(tmp_dir_);
  const auto out_path = tmp_dir_ / "out";
  ...
  EXPECT_EQ(out.count("/sensing/lidar"), 0U);  // not selected
  EXPECT_EQ(out.size(), 2U);
}

TEST_F(TopicKeepTest, KeepInPlaceRewritesInput)
{
  const auto in_path = build_input(tmp_dir_);
  ...
  ASSERT_EQ(result.count("/sensing/lidar"), 1U);
...
TEST_F(TopicKeepTest, KeepAllTopicsRetainsEverything)
{
  const auto in_path = build_input(tmp_dir_);
  const auto out_path = tmp_dir_ / "out";
...
TEST_F(TopicKeepTest, ExistingOutputWithoutOverwriteFails)
{
  const auto in_path = build_input(tmp_dir_);
  const auto out_path = tmp_dir_ / "out";
...
TEST_F(TopicKeepTest, OverwriteReplacesExistingOutput)
{
  const auto in_path = build_input(tmp_dir_);
  const auto out_path = tmp_dir_ / "out";
