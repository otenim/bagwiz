...
// `payload` shares the reader's chunk backing (BagReader::retain_payload)
// instead of copying the bytes; the handle pins that whole chunk while the
// job is in flight, so peak pinned memory is bounded by
// ParallelContext::max_in_flight chunks.
struct DeskewJob
{
  std::size_t seq;
  std::string topic;
  std::int64_t timestamp_ns;
  io::RetainedPayload payload;
...
// `payload` is either the retained input bytes (copy-through and undecodable
// pass-through messages) or a freshly serialized cloud the item owns.
struct OutputItem
{
  std::string topic;
  std::int64_t timestamp_ns;
...
OutputItem process_deskew_job(DeskewJob job, std::span<const core::TrajectoryPose> trajectory)
{
  ...
  item.in_bytes = static_cast<std::uint64_t>(job.payload.payload.size());

  try {
    auto parsed = core::pointcloud::parse_pointcloud2(job.payload.payload);
    if (!parsed.ok()) {
      BAGWIZ_LOG_WARN(
      ...
      if (!res.ok()) {
        ...
        auto serialized = std::make_shared<std::vector<std::byte>>(
          core::pointcloud::serialize_pointcloud2(*res.cloud));
        const std::span<const std::byte> view(serialized->data(), serialized->size());
        item.payload = io::RetainedPayload{view, std::move(serialized)};
      }
    }
    ...
    try {
      while (true) {
...

        {
          ...
          writer.write(item.topic, item.timestamp_ns, item.payload.payload);
        }
        write_prof.add_message(
          item.in_bytes, static_cast<std::uint64_t>(item.payload.payload.size()));
      }
  ...
  try {
    while (true) {
      ...
      if (is_pcd) {
        ...
        {
          // Retention shares the reader's chunk backing instead of copying
          // the payload; it stays on this reader thread's critical path, so
          // it is charged to the read stage.
          auto s = read_prof.time(core::pipeline::Stage::kRead);
          job.payload = rd->retain_payload(raw);
        }
        ...
        // Copy-through message: retain the reader's chunk backing and hand
        // it to the collector through the in-order completion map. The
        // handle keeps the bytes alive until the collector writes and drops
        // the item, so no payload copy and no reader-blocking rendezvous is
        // needed.
        OutputItem item;
        item.topic = name;
        item.timestamp_ns = raw.timestamp_ns;
        {
          // Charged to the read stage for the same reason as the pcd
          // retention above.
          auto s = read_prof.time(core::pipeline::Stage::kRead);
          item.payload = rd->retain_payload(raw);
        }
        item.in_bytes = static_cast<std::uint64_t>(item.payload.payload.size());
