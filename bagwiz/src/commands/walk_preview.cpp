...
void ImagePreviewSession::render_help(std::ostream & out, core::tui::Size term)
{
  ...
  const std::vector<std::string> footer = core::tui::wrap_to_width("  [Esc/q] back", cols);
...
ImagePreviewSession::Exit ImagePreviewSession::run()
{
  ...
  while (running) {
    ...
    if (show_help_) {
      // The overlay accepts only its own keys: Esc or q closes it ('?' only
      // opens it), the scroll keys page it, resize repaints it, and every
      // other key is swallowed so a reference lookup cannot act behind the
      // card. The one exception is Ctrl-C / Ctrl-D, which terminates the
      // session from any screen, overlays included.
      switch (ev) {
    ...
    switch (ev) {
      ...
      case core::KeyEvent::kBack:
      case core::KeyEvent::kQuitView:
        // Esc or q backs out of the preview to the YAML view (the YAML
        // view's own q then quits walk; Esc is absorbed there).
        running = false;
        break;
      case core::KeyEvent::kQuit:
        ...
        break;
      default:
        // Scroll / expand keys are inert in the preview.
        break;
    }
  }
