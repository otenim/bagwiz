...
// per occurrence, so its `<topic>` half completes (as `<topic>=`) at every
// value in its run, until the cursor moves past `=` onto the `<value>` half.
// Its candidates mirror the command's resolution scope: once `--pcd` values
// are on the line, only the PointCloud2 topics those selectors match are
// offered; before any `--pcd` value, all of the bag's PointCloud2 topics.
// `--frame`, `--tolerance`, and `-o`/`--output` take
