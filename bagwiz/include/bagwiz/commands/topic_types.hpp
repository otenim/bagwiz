...
// Image types the shared to_packed_raster() decoder handles — `generate video cam`
...
// `generate video cam --cam-info` and `walk --cam-info` both go through.
...
// bagwiz/src/commands/generate_video_common.cpp (`generate video cam --pcd`) and
