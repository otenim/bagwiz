...
| [`bagwiz movify`](docs/commands/movify.md)     | Render a rosbag to video: camera panels (`--cam`), point-cloud panels (`--pcd`, in 3D and/or from above) and a GNSS map panel (`--gnss`) composed into one grid, with point clouds projected onto the camera panels (`--cam-pcd`) and the vehicle's trajectory drawn over both (`--pose`).                                                                                      |
