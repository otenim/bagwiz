...
| [`bagwiz generate`](docs/commands/generate.md) | Generate non-rosbag media from a rosbag — render image topics to a video (`video cam`), as a single view or a multi-view grid with per-view point-cloud overlays, or a point-cloud topic's scan pattern to a firing-order animation (`video scan`).                                                                                                                             |
