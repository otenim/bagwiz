...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`bagwiz info`](docs/commands/info.md)         | Show a ROS 2 rosbag's bag-level metadata in one screen: layout, storage backend, compression, file count and on-disk size, `metadata.yaml`, topic count, message count, start, end and duration, all read from the bag's own summaries without scanning messages (`-l` lists the shards, `-b` for raw bytes).                                                                   |
| [`bagwiz ls`](docs/commands/ls.md)             | List topics in a ROS 2 rosbag (add `-l` for per-topic counts and average frequencies).                                                                                                                                                                                                                                                                                          |
| [`bagwiz du`](docs/commands/du.md)             | Report per-topic on-disk sizes in a ROS 2 rosbag, du(1)-style: sorted by size descending, each row showing its share of the total, with a `total` row (human-readable sizes by default, `-b` for raw bytes, `-d` for topic-name depth grouping). Compressed containers are split among their topics proportionally.                                                             |
...
| [`bagwiz compress`](docs/commands/compress.md) | Compress or decompress a ROS 2 rosbag: MCAP chunk compression (zstd/lz4), rosbag2 MESSAGE/FILE-mode for SQLite3, or back to plain storage with `-d` / `--mode none`.                                                                                                                                                                                                            |
...
| [`bagwiz movify`](docs/commands/movify.md)     | Render a rosbag to video: camera panels (`--cam`), point-cloud panels (`--pcd`, in 3D and/or from above) and a GNSS map panel (`--gnss`, the track over OpenStreetMap) composed into one grid, with point clouds projected onto the camera panels (`--cam-pcd`) and the vehicle's trajectory drawn over both (`--pose`).                                                        |
| [`bagwiz video`](docs/commands/video.md)       | Convert image topics to video topics and back: encode `Image` / `CompressedImage` topics into `foxglove_msgs/msg/CompressedVideo` (H.264/H.265, one message per frame, playable in Foxglove) with `encode`, or decode such topics back into `CompressedImage` (jpeg/png) or `Image` (bgr8) with `decode`.                                                                       |
...
| [`bagwiz pcd`](docs/commands/pcd.md)           | PointCloud2 topic processing: concatenate multiple LiDAR topics into one (`concat`) via static TF + time sync, motion-deskew one or more topics from an external pose or twist topic (`undistort`), or Draco-compress PointCloud2 topics into CompressedPointCloud2 and back (`compress` / `decompress`).                                                                       |
| [`bagwiz map`](docs/commands/map.md)           | LiDAR and camera map generation and viewing: `map slam`, `map viewer`. Optional build.                                                                                                                                                                                                                                                                                          |
