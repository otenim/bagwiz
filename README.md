...
| [`bagwiz du`](docs/commands/du.md)             | Report per-topic on-disk sizes in a ROS 2 rosbag, du(1)-style: sorted by size descending, with percentages relative to the input bag and a `total` row (human-readable sizes by default, `-b` for raw bytes, `-d` for topic-name depth grouping). A filtered `total` therefore shows its share of the original bag.                                                             |
