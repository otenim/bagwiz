...
# scalars. LD_LIBRARY_PATH is reproduced exactly as `pixi run` would set it
# (pixi prepends and keeps the caller's tail, detected through a sentinel
# seeded into the variable during capture). AMENT_PREFIX_PATH deliberately
# departs from pixi: pixi rebuilds it and discards the caller's value, which
# would silently break sourced message overlays, so the cache always layers
# the captured value in front of the caller's live value instead. Every value
# is `printf %q`-quoted so it round-trips through `source` unchanged, even if
# the checkout path contains shell metacharacters.
set -eu
...
# Emit LD_LIBRARY_PATH into the cache, reproducing pixi's behavior as
# detected via the sentinel seeded into the variable during capture.
# (AMENT_PREFIX_PATH is emitted separately in regenerate_snapshot: it is
# always layered, never sentinel-detected — see there.)
...
regenerate_snapshot() {
    ...
    # Strip a trailing capture sentinel from AMENT_PREFIX_PATH before it is
    # written into the cache: today pixi rebuilds the variable so the sentinel
    # never survives, but a pixi that someday prepends (as it already does for
    # LD_LIBRARY_PATH) would otherwise bake the sentinel into the cache as a
    # bogus search prefix. A bare sentinel (pixi added nothing) is deliberately
    # left for the refusal check below to reject.
    ament="${ament%":${sentinel_ament}"}"
    ...
    if ! {
        ...
        # AMENT_PREFIX_PATH is always emitted in layered form, deliberately
        # departing from pixi: pixi rebuilds it during capture (the sentinel
        # never survives) and discards the caller's value, but the launcher
        # must not — users source message overlays before running bagwiz, and
        # the resolver / decoder find those packages through AMENT_PREFIX_PATH
        # (see README "Using your own message packages (overlays)"). The
        # captured pixi value stays first so it wins on package-name
        # collisions (e.g. a stray foreign-distro prefix sourced by the
        # caller). `ament` is guaranteed non-empty and sentinel-free by the
        # refusal check and the sentinel strip above.
        # The ${AMENT_PREFIX_PATH:+...} suffix is written literally for the
        # cache to expand when sourced (it layers onto the caller's live
        # value), not here.
        # shellcheck disable=SC2016
        printf 'export AMENT_PREFIX_PATH=%s${AMENT_PREFIX_PATH:+:${AMENT_PREFIX_PATH}}\n' \
            "$(shquote "${ament}")"
...
# time. No-op for core builds (e.g. build-core): the directory simply does not exist.
...
# A GPU build (a *-cuda environment) additionally has the CUDA stack at
...
# the directory's presence — only a *-cuda env's install dir has it, so a CPU
