...
// the returned pointer as an identity. cached_record_ns() is the identity that
// actually changes; the pointer only aliases the cache entry.
TEST_F(PointCloudFetcherTest, CachedRecordNsIdentifiesTheCloud)
{
  const std::string topic = "/points";
  const std::vector<CloudSpec> specs{
...
TEST_F(PointCloudFetcherTest, FetchSharedCachesAcrossAlternatingTargets)
{
  const std::string topic = "/points";
  const std::vector<CloudSpec> specs{
    {1'000'000'000, 1'000'000'000, 10.0f},
  ...
  std::string error;
  const auto a1 = fetcher.fetch_shared(1'000'000'000, PointCloudMatchKey::kRecordTime, error);
  ASSERT_NE(a1, nullptr) << error;
  EXPECT_FLOAT_EQ(first_point_x(*a1), 10.0f);

  const auto b = fetcher.fetch_shared(2'000'000'000, PointCloudMatchKey::kRecordTime, error);
  ASSERT_NE(b, nullptr) << error;
  EXPECT_FLOAT_EQ(first_point_x(*b), 20.0f);

  // Matching the first cloud again must return the same cached instance; a
  // one-cloud cache would have evicted it when the second cloud loaded.
  const auto a2 = fetcher.fetch_shared(1'000'000'000, PointCloudMatchKey::kRecordTime, error);
  ASSERT_NE(a2, nullptr) << error;
  EXPECT_EQ(a1, a2);
  // Shared ownership kept the first cloud valid across the later fetches.
  EXPECT_FLOAT_EQ(first_point_x(*a1), 10.0f);
}
