...
TEST(Projector, IgnoresTrailingPointsBeyondTheDeclaredCount)
{
  // The blob holds two valid points but the header declares one: only the
  // declared point projects. Fails if the walk ever stops clamping to
  // height * width.
  auto cloud = make_xyz_cloud({{0.0f, 0.0f, 5.0f}, {0.0f, 0.0f, 6.0f}});
  cloud.width = 1;
