...
| `BAGWIZ_BUILD_TYPE`        | a CMake build type (default `Release`)                                                       | CMake build type for `pixi run build-core`/`build-full` when the positional build-type arg is omitted; the positional arg wins when both are given.                                                                                                                                   | `scripts/bagwiz-build.sh`    |
| `BAGWIZ_BUILD_PARALLELISM` | a positive integer (default: half the host's physical CPU cores)                             | Value forwarded to colcon's `--parallel-workers` by `pixi run build-core`/`build-full` when the positional worker-count arg is omitted; the positional arg wins.                                                                                                                      | `scripts/bagwiz-build.sh`    |
