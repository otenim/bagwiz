...
| `-j`, `--threads <N>`         | Number of worker threads for Pass 2. Default: `0`, which uses `std::thread::hardware_concurrency()` — extra deskew workers cost no additional CPU now that the pipeline is copy-free, so the default takes every core. Accepts `0`–`256`; `1` forces the synchronous path; in-range values above hardware concurrency are capped to it. |
