...
| `--lossless`            | Store attributes unquantized so floats round-trip bit-exactly. Default: lossy, 14-bit quantization per attribute type. Mutually exclusive with the `--*-bits` flags. Clouds containing NaN/Inf values are compressed only in this mode; in lossy mode they pass through uncompressed (see [Skip and pass-through rules](#skip-and-pass-through-rules)). |
