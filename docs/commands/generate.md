...
| `--range <m>`             | BEV half-extent in meters: the BEV view spans +-range on both ground axes. In the 3D view it only sets the default `--dist` (1.5x the range). Default: auto — the largest finite XY distance in the first cloud. Long-form only.                           |
...
| `--dist <m>`              | 3D view: camera distance from the sensor in meters. Default: 1.5x the range. Long-form only.                                                                                                                                                               |
