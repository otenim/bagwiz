...
bagwiz calib cam-lidar -i <input> --pcd <topic> --pose <topic> --cam <topic> \
  ...
  [--cam-offset <dur>|auto] [--imu <topic>] [-j|--threads <N>] [--json] \
  [-w|--overwrite]
```
...
| `--cam-info <topic>`    | CameraInfo topic. When omitted, resolved from `--cam` using the same auto-resolution rules as [`generate video cam`'s `--cam-info`](generate.md#bagwiz-generate-video-cam). A literal topic name, not a glob. Long-form only.                                                                                                                                                                                                                                                                 |
...
| `-j`, `--threads <N>`   | Number of worker threads for the map accumulation, the refinement and the sample decoding. Default `0`, which uses `std::thread::hardware_concurrency()`; `1` runs everything on the calling thread. Accepts `0`–`256`; in-range values above hardware concurrency are capped to it. The thread count never changes the result: the map is filled in the same order and the NID histograms count the same integers for every split, so the YAML and the report are identical for every value. |
