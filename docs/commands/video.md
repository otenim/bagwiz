...
| `--preset <preset>`         | Speed/quality preset, by libx264's names: `ultrafast`, `superfast`, `veryfast`, `faster`, `fast`, `medium`, `slow`, `slower`, `veryslow`. libx265 takes the same names; NVENC maps them onto its `p1` (fastest) to `p7` (slowest). Default: `medium`. Long-form only.                                                                                                                                |
