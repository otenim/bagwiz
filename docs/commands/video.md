...
| `--preset <preset>`         | Encoder effort: `fastest`, `faster`, `default`, `slower`, or `slowest`. Slower presets spend more encoding time to produce smaller output at the same quality target. They map to libx264/libx265 `ultrafast`, `veryfast`, `medium`, `slower`, and `veryslow`, and to NVENC `p1`, `p3`, `p4`, `p6`, and `p7`, respectively. Default: `default`. Long-form only.                                      |
