...
| `--keep-inputs`             | Keep the source image topics in the output next to the video topics. Default: replace them. Long-form only.                                                                                                                                                                                                                                                                                          |
| `--codec <codec>`           | `h264` (widest playback support) or `h265` (smaller at the same quality; Foxglove needs a browser or desktop build with HEVC decoding, see [Playback in Foxglove](#playback-in-foxglove)). Default: `h264`. Long-form only.                                                                                                                                                                          |
| `--encoder <encoder>`       | `auto` uses NVIDIA NVENC for frames larger than 1080p when this build and a GPU support it, else the CPU encoder (libx264 / libx265); `cpu` and `nvenc` force one. NVENC's H.264 encoder (not H.265) tops out at 4096x4096 on most GPUs; a forced `nvenc` encode of a larger H.264 frame fails outright, while `auto` falls back to the CPU encoder with a warning. Default: `auto`. Long-form only. |
| `--preset <preset>`         | Speed/quality preset, by libx264's names: `ultrafast`, `superfast`, `veryfast`, `faster`, `fast`, `medium`, `slow`, `slower`, `veryslow` (the same set `movify --preset` takes). libx265 accepts the same names; NVENC maps them onto its `p1`-`p7`. Slower presets spend more encoding time to produce smaller output at the same quality target. Default: `medium`. Long-form only.                |
| `--crf <n>`                 | Constant-quality target, `0` (best) to `51` (smallest); NVENC uses it as its constant-quality level. Default: `23` for `h264`, `28` for `h265` (whose scale sits about five points lower for a similar picture). Long-form only.                                                                                                                                                                     |
| `--gop <n>`                 | Keyframe interval in frames: every `n`-th frame is a keyframe a player can start or seek from. A smaller value seeks faster and costs bytes. Must be at least 1. Default: `30`. Long-form only.                                                                                                                                                                                                      |
...
| `-w`, `--overwrite`         | Overwrite an existing `-o/--output` path. Has no effect in in-place mode.                                                                                                                                                                                                                                             |
| `--keep-inputs`             | Keep the source video topics in the output next to the image topics. Default: replace them. Long-form only.                                                                                                                                                                                                           |
| `--format <format>`         | The message written per frame: `jpeg` or `png` (`sensor_msgs/msg/CompressedImage` with that `format`) or `raw` (`sensor_msgs/msg/Image`, `bgr8`, `step = width * 3`). Default: `jpeg`. Long-form only.                                                                                                                |
| `--quality <n>`             | JPEG quality for `--format jpeg`, `1` (smallest) to `100` (best). Ignored by the other formats. Default: `90`. Long-form only.                                                                                                                                                                                        |
