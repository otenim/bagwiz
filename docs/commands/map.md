...
| `--recovery-min-inliers` | Inlier-fraction acceptance gate (`0`–`1`, default `0.7`) for warmup/cooldown pose recovery scan-matching. Higher = stricter (endpoints may stay unrecovered); lower = looser (a bad fit can degrade recovery). No effect when both recoveries are disabled. |
