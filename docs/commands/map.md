...
| `--remove-dynamic`             | Remove ghost points left by moving objects from the finished map (DUFOMap-style void-region ray casting): every scan's rays mark the voxels they traverse as seen-free, and a scan point falling in a voxel that was ever seen free is dropped before the map merge. Requires `--pcd` (it ray-casts the LiDAR scans). Off by default (the exported map is unchanged without it). Filters `map.pcd` only; the trajectory is untouched. Runs multithreaded under `--threads`, and the kept/dropped decision is identical for any `--threads` value, because the free-space marking is a monotone OR — a reduction that is exactly commutative and associative.                                                                                                                                                                                                                                                                                                                |
