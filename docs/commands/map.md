...
| ------------------------------ | --------------------------------------------------------------------------------------------------------------------- |
| [`slam`](#bagwiz-map-slam)     | Estimate a trajectory (and map) from a PointCloud2 topic, or camera-only visual-inertial SLAM from `--cam` + `--imu`. |
...
Cameras can join in two independent roles: as colorization sources (`--color`)
and as SLAM constraints (`--cam`). Without `--pcd`, `--cam` + `--imu` alone run
camera-only visual-inertial SLAM — see [Camera-only mode](#camera-only-mode---cam-without---pcd).
...
Either `--pcd` (LiDAR SLAM) or `--cam` (camera-only visual-inertial SLAM) must
be given; both may be.
...
# Camera-only visual-inertial SLAM (no --pcd): the cameras drive the odometry;
# map.pcd is the sparse rgb landmark map. --imu is required in this mode.
bagwiz map slam -i drive.mcap -o out/ \
  --cam /sensing/camera/camera6/image_raw/compressed \
        /sensing/camera/camera7/image_raw/compressed \
  --imu /sensing/imu/imu_data
...
| `--pcd <pcd_topic>`                 | `sensor_msgs/msg/PointCloud2` topic to run SLAM on. Optional since camera-only mode: either `--pcd` or `--cam` must be given (both may be); without `--pcd` the run is camera-only visual-inertial SLAM driven by `--cam` + `--imu`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `-o`, `--output <output_root>`      | **Required.** Output directory. Receives `traj.tum` and `map.pcd`. Created if absent.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `--imu <topic>`                     | `sensor_msgs/msg/Imu` topic. Switches odometry to LiDAR-IMU (GLIM's `OdometryEstimationCPU`, or `OdometryEstimationGPU` when the GPU backend is active — `--backend cuda`, or the default `auto` on a CUDA build with a visible device). The LiDAR←IMU extrinsic is resolved from the bag's static TF using the cloud and IMU header `frame_id`s (or, in camera-only mode, the first `--cam` camera's frame). Required in camera-only mode (`--cam` without `--pcd`): the visual-inertial odometry integrates the IMU stream.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `--visual-anchor-period <duration>` | Camera-only anchor-window period as `<number><unit>` (e.g. `100ms`, `0.1s`; must be positive; requires `--cam`, rejected with `--pcd`): the cross-camera grouping window keyed to the first `--cam` camera's frames — see [Camera-only mode](#camera-only-mode---cam-without---pcd). Default: derived from that topic's median inter-frame interval (logged at startup); pass the flag only when the derivation is wrong for the bag (heavy stamp irregularity).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `--visual-final-ba`                 | Camera-only final batch bundle adjustment (requires `--cam`, rejected with `--pcd`): after the global optimization, re-estimate every keyframe against ALL visual observations and the raw IMU stream recorded during the run in one batch solve of the visual-inertial odometry's factor graph (smart projection factors over every track plus IMU preintegration factors between consecutive keyframes), seeded from the globally-optimized trajectory. This refines the trajectory's endpoints — the first keyframes, pinned to the IMU-only gravity alignment, and the last ones, which carry only young feature tracks and are never re-solved with future data — and then re-triangulates the sparse landmark map at the refined poses. Off by default; deterministic.                                                                                                                                                                                                                                                                                                                             |
...
| `--input-res <m>`                   | Voxel size in meters (must be positive) used for **both** the GLIM LiDAR input downsample and the exported-map merge — the single map-resolution knob. Smaller = denser map and finer SLAM detail, at the cost of more points and runtime. Unlike a pure export voxel it feeds the optimizer, so changing it also changes the trajectory (not just the map's appearance). Has no effect in camera-only mode (no LiDAR downsample, and the sparse landmark map is not voxel-merged). The range crop (`--min-range`/`--max-range`) still bounds which returns enter the pipeline. Default: `0.15` (GLIM's stock downsample, so the default trajectory matches GLIM's).                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
### Camera-only mode (`--cam` without `--pcd`)

With no `--pcd`, the run is visual-inertial SLAM: the `--cam` cameras drive a
visual-inertial odometry (multi-camera KLT feature tracking fused with the IMU
in a rebuilt-window batch smoother) instead of a LiDAR backend, while the
SubMapping → GlobalMapping pipeline and the `--cam` co-visibility constraints
work unchanged. `--imu` is required — gravity alignment and the asynchronous
multi-camera folding both integrate the IMU stream — and a single camera is
enough (monocular VIO). `--gnss` stays available as an additive constraint.

Differences from the LiDAR modes:

- The first `--cam` topic's camera frame takes the cloud frame's role: every
  static-TF resolution (the IMU and camera extrinsics, the GNSS lever-arm,
  `--frame`) anchors at it, and `traj.tum` expresses that camera's pose by
  default. That camera is also the grouping anchor: the other cameras'
  observations join windows keyed to its frames, one window per frame period.
  The period is derived from the topic's median inter-frame interval
  (`--visual-anchor-period` overrides it).
- `map.pcd` is the sparse rgb landmark map: every feature track that survived
  triangulation, re-triangulated once at the final optimized poses (no
  duplicates) and colored from its own track observations. There is no dense
  point cloud and no `--color` pass in this mode.
- `--color`, `--remove-dynamic`, and `--remove-outliers` require `--pcd` and
  are rejected; `--backend cuda` is unsupported (the visual-inertial odometry
  is CPU-only); the trajectory endpoint fills are off (no scans to fill with).
  The camera-only counterpart to the fills is `--visual-final-ba`: rather
  than covering missing poses, it refines the estimated ones — see
  [Trajectory endpoint fill](#trajectory-endpoint-fill).
- Expected accuracy is VIO-grade, not LiDAR-grade: the goal is a usable
  trajectory, not parity with the LiDAR map. When comparing against the LiDAR
  run of the same bag as pseudo-ground-truth, use SE(3) alignment only — a
  Sim(3) fit would hide exactly the scale error the comparison is meant to
  expose.
- The odometry estimates its initial orientation by averaging the opening
  ~1 s of IMU data, which assumes the platform is stationary at the start of
  the fed stream (GLIM's `NaiveInitialStateEstimation` — the only
  point-cloud-free initializer). Starting mid-motion (or trimming a bag to a
  moving section) tilts the initialization and degrades the whole trajectory;
  start the bag from a stationary section.

Camera time offsets and rolling shutter are not modeled (the header stamp is
trusted) — the same assumption the `--cam` constraints and `--color`
colorization make.
...
or, in camera-only mode, the first `--cam` camera's frame — (the sensor
frame). Passing `--frame` resolves the sensor → target transform from
...
LiDAR modes only — camera-only mode has no scans to fill with, so the fills
are off there regardless of these flags. Its counterpart is `--visual-final-ba`:
rather than filling missing poses, it refines the estimated ones — a final
batch bundle adjustment that re-estimates every keyframe against all visual
observations and the IMU stream (smart projection factors plus IMU
preintegration factors) after the global optimization, lifting the
trajectory's first and last keyframes (pinned to the IMU-only gravity
alignment at the start, dead-reckoned over young feature tracks at the end)
up toward mid-trajectory accuracy and re-triangulating the landmark map at
the refined poses.
...
failed to decode or track)` line, a per-camera `Visual timing '<topic>': …`
breakdown of where the tracking time went (decode, KLT, detection) and how
long the bag reader waited on that camera, a `visual constraints: <N> factors
from <M> tracks over <K> submaps (dropped: ...) in <T>s` breakdown from the
...
field (written by `map slam --color`, or by a camera-only `map slam` run) additionally offers `rgb` — the true
