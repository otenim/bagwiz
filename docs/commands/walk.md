...
| `--cam-info <topic>`    | Long-form only. Explicit `sensor_msgs/msg/CameraInfo` topic for the preview's rectify toggle and the PointCloud2 projection overlay. A literal topic name, not a glob. When omitted, bagwiz auto-resolves it from `<topic>` using the rules documented for `bagwiz movify --cam-info`. |
...
- **Supported encodings mirror `bagwiz movify`:** raw
...
number of topics; their projected points are drawn together. Press `Esc`
...
camera and lidar, and match what `bagwiz movify --cam-pcd` renders for the
...
compact line: `rect` and `pcd` badges while those states are on, plus any
...
disagree.
The footer shows only the working set; the adjustment keys
(`f`/`c`/`r`/`=`/`-`/`[`/`]`) stay live and are listed in the `?` reference.
...
| `S`                 | Save as yaml - writes the current message body (prompts for path). In the image preview, `S` saves the decoded image as a PNG file, including any rectification or point-cloud overlay.                                         |
