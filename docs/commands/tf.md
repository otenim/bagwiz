...
and their subtrees), [`dump`](#bagwiz-tf-static-dump) (write the static tree as
a publisher-config YAML), [`join`](#bagwiz-tf-static-join) (embed such a YAML
into a bag), and [`update`](#bagwiz-tf-static-update) (add or update individual
edges), so the full invocation is `bagwiz tf static calc ...`. Running
